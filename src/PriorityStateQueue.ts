...
export default class PriorityStateQueue {
    ...
    private checkHeapIndex(index: number, heap: QueueTicket[]) {
        if (index < 0) {
    ...
    private getLeftChildIndex(index: number, heap: QueueTicket[]) {
        this.checkHeapIndex(index, heap);
        const value = 2 * index + 1;
        return value < length ? value : null;
    }

    private getRightChildIndex(index: number, heap: QueueTicket[]) {
        this.checkHeapIndex(index, heap);
        const value = 2 * index + 2;
        return value < length ? value : null;
    }

    private getParentIndex(index: number, heap: QueueTicket[]) {
        this.checkHeapIndex(index, heap);
        if (index === 0) {
            return null;
        }
    ...
    private swapHeapElements(index1: number, index2: number, heap: QueueTicket[]) {
        this.checkHeapIndex(index1, heap);
        this.checkHeapIndex(index2, heap);
    ...
    private heapify(index: number, heap: QueueTicket[]) {
        ...
        let largest = index;
        const left = this.getLeftChildIndex(index, heap);
        const right = this.getRightChildIndex(index, heap);
        ...
        if (largest !== index) {
            this.swapHeapElements(largest, index, heap);
    ...
    insert(ticket: QueueTicket, layerName: Layer = defaultLayerName, priority: Priority = defaultPriority) {
        ...
        if (!layerMap.has(priority)) {
            ...
            let parent = this.getParentIndex(index, layerHeap);
            // Bubble up
            while (parent) {
                if (layerHeap[parent] < layerHeap[index]) {
                    this.swapHeapElements(parent, index, layerHeap);
                } else {
                    break;
                }
                index = parent;
                parent = this.getParentIndex(index, layerHeap);
            }
        }
